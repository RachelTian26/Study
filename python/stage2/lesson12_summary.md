...
- `JSON` 是最常见的数据格式之一，很多网站、APP、接口和配置文件都在用它。 |
- 它能把 Python 里的 `dict` 和 `list` 直接保存成文件，程序关掉以后还可以再次读取。 |
- 这让程序有了“记忆”，不再只是一次性的计算结果。 |
- 以后做通讯录、待办列表、日历、用户信息、AI 返回数据时，几乎都会碰到 `JSON`。 |
- `JSON` 也能在不同语言之间传递数据，比如 Python 和 JavaScript 之间都能互相读写。 |
- 如果不学 `JSON`，你以后做数据处理和项目开发时，会非常难处理程序里保存的数据。 |
- 学会 `JSON` 之后，你就能更自然地写“保存数据”“读取数据”“继续使用数据”的程序。 |
